...
def eliminate_iff(formula: Formula) -> Formula:
    ...
    """
    # Recorre la formula y reemplaza cada bicondicional por dos implicaciones.
    if isinstance(formula, Atom):
        return formula
    if isinstance(formula, Not):
...
def eliminate_implication(formula: Formula) -> Formula:
    ...
    """
    # Recorre la formula y reemplaza cada implicacion por una disyuncion.
    if isinstance(formula, Atom):
        return formula
    if isinstance(formula, Not):
...
def push_negation_inward(formula: Formula) -> Formula:
    ...
    """
    # Empuja las negaciones hasta que solo queden sobre atomos.
    if isinstance(formula, Atom):
        return formula
    if isinstance(formula, Not):
...
def distribute_or_over_and(formula: Formula) -> Formula:
    ...
    """
    # Distribuye recursivamente hasta que toda disyuncion sea una clausula.
    ...
    if isinstance(formula, Or):
        ...
        for i, disjunct in enumerate(disjuncts):
            if isinstance(disjunct, And):
                ...
                distributed = [
                    distribute_or_over_and(Or(*(others + [conjunct])))
...
def flatten(formula: Formula) -> Formula:
    ...
    """
    # Aplana conjunciones y disyunciones anidadas sin cambiar la estructura restante.
    if isinstance(formula, Atom):
        return formula
    if isinstance(formula, Not):
    ...
    if isinstance(formula, And):
        ...
        for conjunct in formula.conjuncts:
            ...
            if isinstance(flat_conjunct, And):
                conjuncts.extend(flat_conjunct.conjuncts)
            else:
                conjuncts.append(flat_conjunct)
        if len(conjuncts) == 1:
            return conjuncts[0]
    ...
    if isinstance(formula, Or):
        ...
        for disjunct in formula.disjuncts:
            ...
            if isinstance(flat_disjunct, Or):
                disjuncts.extend(flat_disjunct.disjuncts)
            else:
                disjuncts.append(flat_disjunct)
        if len(disjuncts) == 1:
            return disjuncts[0]
        return Or(*disjuncts)
    return formula
